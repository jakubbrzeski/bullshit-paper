...
public class MainWindowController implements Initializable {
    ...
    @FXML
    private ComboBox<ParsersProvider.ParserObject> _portalComboBox;
    @FXML
    ...
    private final ParsersProvider parsersProvider = new ParsersProvider();
    private final SectionInfo _newSection = new SectionInfo(new ArrayList<String>(), "[New section]",
            java.awt.Color.RED, false, parsersProvider.getParsers().get(0));

    @Override
    public void initialize(URL url, ResourceBundle rb) {
        _portalComboBox.setItems(FXCollections.observableArrayList(parsersProvider.getParsers()));
        _portalComboBox.setValue(parsersProvider.getParsers().get(0));
        _sectionsLV.setItems(_sectionsNames);
        _sectionsLV.getSelectionModel().selectedIndexProperty().addListener(new SectionUpdater());
        _sectionControls = new Control[]{_removeSectionBt, _sectionNameTF, _sectionTagsTF, _sectionHeaderColorPicker, _sectionSudokuCB, _portalComboBox};
    ...
    private static class GeneratePaperTask extends Task<Void> {
        ...
        public GeneratePaperTask(File file, String title, List<SectionInfo> sections,
                                 IArticleParser parser, IArticleProvider provider) {
            _file = file;
            _title = title;
            _sections = sections;
        }
    ...
    private SectionInfo saveSection() {
        ...
        return new SectionInfo(tags, _sectionNameTF.getText(), headerColor, _sectionSudokuCB.isSelected(), _portalComboBox.getValue());
    }
    ...
    private void loadSection(SectionInfo si) {
        ...
        _portalComboBox.setValue(si.getParserObject());
    }
}
